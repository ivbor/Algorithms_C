...
static void test_minknap_compatibility_wrapper(void) {
    const int profits[] = {20, 5, 10, 40, 15, 25};
    const int weights[] = {1, 2, 3, 8, 7, 4};
    int selected[] = {0, 0, 0, 0, 0, 0};

    int best_direct = ac_minknap_max_profit(6, profits, weights, 10, selected);
    int best_compat = ac_minknap(6, profits, weights, NULL, 10);

    MU_ASSERT(best_direct == 60);
    MU_ASSERT(best_compat == best_direct);
}
...
int main(void) {
    ...
    run_test(test_minknap_compatibility_wrapper);
