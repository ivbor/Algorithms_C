...
static int run_slist_stress(void) {
    ...
    for (size_t iteration = 0; iteration < SLIST_ITERATIONS; ++iteration) {
        ...
        switch (operation) {
            ...
            case 3: { // pop_front
                int actual = 0;
                if (ref.size == 0) {
                    if (ac_slist_pop_front(&list, &actual) != AC_SLIST_ERR_EMPTY) {
                        fprintf(stderr, "SList pop_front should report empty\n");
                        goto slist_error;
                    }
                    stats[3].expected_failures++;
                } else {
            ...
            case 4: { // pop_back
                int actual = 0;
                if (ref.size == 0) {
                    if (ac_slist_pop_back(&list, &actual) != AC_SLIST_ERR_EMPTY) {
            ...
            case 5: { // erase arbitrary
                int actual = 0;
                if (ref.size == 0) {
                    if (ac_slist_erase(&list, 0, &actual) != AC_SLIST_ERR_EMPTY) {
                    ...
                    if (ac_slist_erase(&list, index, &actual) != AC_SLIST_OK ||
                        ref_vector_erase(&ref, index) != 0 || expected != actual) {
            ...
            case 6: { // contains
                ...
                bool actual = ac_slist_contains(&list, &needle, compare_ints, NULL);
                if (actual != expected) {
    ...
    printf("Singly linked list stress test passed (%d iterations).\n",
           SLIST_ITERATIONS);
    return 0;
