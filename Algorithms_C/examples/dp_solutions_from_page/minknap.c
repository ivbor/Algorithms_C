...
void *palloc(long size) {
    ...
    if (size == 0)
        size = 1;
    if (size != (size_t)size)
        errorx("Alloc too big %ld", size);
    p = malloc(size);
    if (p == NULL)
        errorx("no memory size %ld", size);
    return p;
}
...
void improvesolution(allinfo *a, state *v) {
    if (v->wsum > a->c)
        errorx("wrong improvesoluton");
    if (v->psum <= a->z)
        errorx("not improved solution");
...
void partsort(allinfo *a, item *f, item *l, stype ws, int what) {
    ...
    d = l - f + 1;
    if (d < 1)
        errorx("negative interval in partsort");
    if (d > MINMED) {
