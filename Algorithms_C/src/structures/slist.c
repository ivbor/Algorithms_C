 ...
 * control flow even when terser idioms exist.  The prose intentionally outweighs
 * the code so that the design decisions remain obvious to readers who compare
...
static ac_slist_node *ac_slist_allocate_node(
    const ac_slist *list,
    const void *element
) {
...
int ac_slist_for_each_const(
    ...
    for (const ac_slist_node *node = list->head; node != NULL; node = node->next) {
        int status = visit(node->data, user_data);
        if (status != 0) {
            return status;
        }
