...
void printMatrix(int **matrix, int rows, int columns, int indexes) {
    ...
    if (!to_print) {
        log_message(LOG_FATAL, strdup("Failed to allocate memory for to_print."));
        exit(EXIT_FAILURE);
    }
    ...
    if (indexes) {
        for (int i = 0; i <= columns; i++) {
            ...
            if (required_length < ELEMENT_BUFFER_SIZE) {
                snprintf(one_element, ELEMENT_BUFFER_SIZE, "%*d ", FORMAT_WIDTH, i);
                to_print = mystrcat(to_print, one_element);
            } else {
                log_message(LOG_FATAL, strdup("Allocated insufficient memory for one_element."));
                log_message(LOG_FATAL, strdup("Difference (required - allocated) = "));
                
                char char_length[12];  // Enough to store any int value
                snprintf(char_length, sizeof(char_length), "%d", required_length - ELEMENT_BUFFER_SIZE);
    ...
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j <= columns; j++) {
            ...
            if (j < columns) {
                snprintf(one_element, ELEMENT_BUFFER_SIZE, "%*d ", FORMAT_WIDTH, matrix[i][j]);
                to_print = mystrcat(to_print, one_element);
            }
        }
        to_print = mystrdup("\n\n", to_print);
